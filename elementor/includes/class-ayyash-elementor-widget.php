...
/**
 * Class Ayyash_Elementor_Widget
 * @package AyyashAddons
 */
class Ayyash_Elementor_Widget extends Widget_Base {
	...
	public function get_name() {
		if ( null === $this->name ) {
			$called_class = str_replace(
				[
					'AyyashAddons\\Widgets\\',
				],
				'',
				get_called_class()
			);
			$called_class = str_replace( 'AyyashAddons_Style_', '', $called_class );
			$called_class = str_replace( '_', '-', $called_class );
			$this->name   = 'ayyash-' . strtolower( $called_class );
		}
	...
	protected function get_base_id() {
		if ( null === $this->base_id ) {
			$this->base_id = str_replace( 'ayyah-', '', $this->get_name() );
		}
	...
	protected function plugin_dependency_alert( $dependencies ) {
		...
		foreach ( $dependencies as $dependency ) {
			...
			foreach ( $dependency['plugins'] as $plugin ) {
				...
				if ( ayyash_addons_is_plugin_active( $plugin['path'] ) ) {
					continue;
				}
	 ...
	 * @return array
	 */
	protected function list_templates( $slug, $path = AYYASH_ADDONS_ELEMENTOR_WIDGETS_PATH ) {
		return [
	...
	protected function list_controller( $slug = '', $path = AYYASH_ADDONS_ELEMENTOR_WIDGETS_PATH ) {
		$controller = [
			sprintf(
	 ...
	 * @return array
	 */
	protected function list_content_templates( $slug, $path = AYYASH_ADDONS_ELEMENTOR_WIDGETS_PATH ) {
		return [
			// @XXX if we introduce template override from the theme, then it would be added on the top of main template.
			sprintf(
